...
import { authenticateCmd, revalidateCmd, signInCmd, signOutCmd, signUpCmd } from "../commands/auth";
  ...
  await fastify.register(
    fp<{ token: string; store: typeof store }>(
      async (fastify, opts) => {
        ...
        bot.command("authenticate", authenticateCmd);
        ...
        bot.action(/^(waitlist):(.+)$/, waitlistCb);
        ...
        bot.on("message", async (ctx, next) => {
          ...
          else if (state === "add_participant") await addParticipantHdlr(ctx, next);
          return await next();
        });
